...
class Economy(commands.Cog):
    ...
    @discord.slash_command(guild_ids=[844842869285847060], name="balance")
    ...
    @discord.slash_command(guild_ids=[844842869285847060], name="work")
    ...
    @discord.slash_command(guild_ids=[844842869285847060], name="coinflip")
    ...
    @discord.slash_command(guild_ids=[844842869285847060], name="steal")
    @commands.cooldown(1, cooldowns["steal"], commands.BucketType.member)
    async def steal(self, ctx: discord.ApplicationContext, member: discord.Option(discord.SlashCommandOptionType.mentionable, name="member", description="The person you want to steal from", required=True)):
        if not isinstance(member, discord.Member):
            await ctx.respond(embed=self.client.create_embed_from_json("incorrect-argument-message", ctx.guild.id, {"%arg_name%": "member", "%arg_type%": "Member", "%t%": "lol"}))
            return
        balance = await self.check_economy(ctx.user.id)
        otherbalance = await self.check_economy(member.id)
        if otherbalance == 0:
            await ctx.respond(embed=self.client.create_embed_from_json("other-balance-not-enough-message", ctx.guild.id, {"%member%": member.mention}))
            return
        chance = random.randint(0, 100)
        win = False
